 ...
 *
 *   Теперь граница кадра определяется по ИЗМЕРЕННОМУ интервалу, а не только по флагу:
 *       gap = переполнение TMR1  ||  (delta >= GAP_TICKS)
 *   delta ловит паузы 4..16.38 мс (которые умещаются в один период таймера),
 *   флаг переполнения ловит паузы > 16.38 мс (где delta заворачивается и неточна).
...
/* TMR1 тактируется Fosc/4 = 4 МГц => 4 тика на 1 мкс, 0.25 мкс на тик. */
#define US_TO_TICKS(us)   ((uint16_t)((us) * 4u))
...
void main(void) {
    ...
    /* TMR1: Fosc/4 = 4 МГц, прескалер 1:1, без прерывания по переполнению.
     * Флаг TMR1IF используем как индикатор «была длинная пауза» при синхронизации. */
    T1CON = 0;
